...
export default function Home() {
  ...
  /* Lenis removed — scrub:0.3 provides all the smoothing needed.
     Lenis + scrub was double-smoothing, causing 4000px of dead scroll. */
  ...
  useGSAP(() => {
    const build = () => {
      ...
      /*
        NORMALIZED TIMING: Every layer gets exactly 1 unit of timeline.
        Within that unit: ~0.3 enter, ~0.4 hold, ~0.3 exit.
        This ensures even distribution across the scroll runway.
      */
      const ENTER = 0.3;
      const HOLD = 0.4;
      const EXIT = 0.3;
      ...
      // Orb parallax
      const totalDur = layers.length * (ENTER + HOLD + EXIT);
      ...
      layers.forEach((layer, i) => {
        ...
        if (i === 0) {
          // ── MONOGRAM: hold briefly then split exit ──
          gsap.set(layer, { opacity: 1 });
          tl.to({}, { duration: HOLD });
          tl.to(layer.querySelector(".mono-n"), { x: -60, opacity: 0, filter: "blur(8px)", duration: EXIT, ease: "none" });
          tl.to(layer.querySelector(".mono-arrow"), { x: 60, opacity: 0, filter: "blur(8px)", duration: EXIT, ease: "none" }, "<");
          tl.to(layer.querySelector(".mono-glow"), { opacity: 0, scale: 1.5, duration: EXIT, ease: "none" }, "<");

        } else if (i === layers.length - 1) {
          // ── CTA: enters and stays ──
          gsap.set(layer, { opacity: 0 });
          if (inner) gsap.set(inner, { y: 40, scale: 0.9, filter: "blur(3px)" });
          tl.to(layer, { opacity: 1, duration: ENTER, ease: "none" });
          if (inner) tl.to(inner, { y: 0, scale: 1, filter: "blur(0px)", duration: ENTER, ease: "none" }, "<");
          ...
          // Enter
          tl.to(layer, { opacity: 1, duration: ENTER * 0.4, ease: "none" });
          if (inner) tl.to(inner, { scale: 1, filter: "blur(0px)", duration: ENTER, ease: "none" }, "<");
          if (split) tl.to(split.lines, { yPercent: 0, opacity: 1, duration: ENTER, stagger: 0.08, ease: "none" }, "<");

          // Hold
          tl.to({}, { duration: HOLD });

          // Exit
          if (split) tl.to(split.lines, { yPercent: -110, opacity: 0, duration: EXIT * 0.8, stagger: 0.04, ease: "none" });
          if (inner) tl.to(inner, { scale: 0.93, filter: "blur(2px)", duration: EXIT, ease: "none" }, "<");
          tl.to(layer, { opacity: 0, duration: EXIT * 0.5, ease: "none" }, "<");

        } else if (isClosing) {
          // ── CLOSING: enters, ZOOMS to 1.25, holds, exits ──
          gsap.set(layer, { opacity: 0 });
          if (inner) gsap.set(inner, { y: 30, scale: 0.85, filter: "blur(4px)" });

          // Enter
          tl.to(layer, { opacity: 1, duration: ENTER * 0.5, ease: "none" });
          if (inner) tl.to(inner, { y: 0, scale: 1, filter: "blur(0px)", duration: ENTER, ease: "none" }, "<");

          // Zoom punch
          if (inner) tl.to(inner, { scale: 1.25, duration: HOLD * 0.5, ease: "none" });

          // Hold at zoom
          tl.to({}, { duration: HOLD * 0.5 });

          // Exit
          if (inner) tl.to(inner, { scale: 0.9, y: -30, filter: "blur(3px)", duration: EXIT, ease: "none" });
          tl.to(layer, { opacity: 0, duration: EXIT * 0.6, ease: "none" }, "<");

        } else {
          // ── CONTENT: body + stat with glass + bloom ──
          gsap.set(layer, { opacity: 0 });
          if (inner) gsap.set(inner, { y: 50, scale: 0.88, filter: "blur(3px)" });
          if (bloom) gsap.set(bloom, { opacity: 0, scale: 0.75 });

          // Enter
          if (bloom) tl.to(bloom, { opacity: 1, scale: 1, duration: ENTER, ease: "none" });
          tl.to(layer, { opacity: 1, duration: ENTER * 0.5, ease: "none" }, "<");
          if (inner) tl.to(inner, { y: 0, scale: 1, filter: "blur(0px)", duration: ENTER, ease: "none" }, "<");
          ...
          // Exit
          if (bloom) tl.to(bloom, { opacity: 0, scale: 0.85, duration: EXIT * 0.6, ease: "none" });
          if (inner) tl.to(inner, { y: -40, scale: 0.92, filter: "blur(2px)", duration: EXIT, ease: "none" }, "<");
          tl.to(layer, { opacity: 0, duration: EXIT * 0.5, ease: "none" }, "<");
        }
      });
    };
        ...
        <div ref={pinnedRef} className="h-dvh w-full overflow-hidden relative bg-canvas">
          ...
          <div className="layer opacity-0 absolute inset-0 flex items-center justify-center will-change-transform z-20" data-layer-quote>
            <div className="layer-inner will-change-transform text-center px-6">
              <h1 data-quote className={`${disp} uppercase text-[clamp(1.8rem,5vw,4rem)] tracking-[0.25em] text-text-primary leading-[1.3]`}>
          ...
          <div className="layer opacity-0 absolute inset-0 flex items-center justify-center will-change-transform z-20" data-layer-closing>
            <div className="layer-inner will-change-transform text-center px-6 max-w-3xl">
